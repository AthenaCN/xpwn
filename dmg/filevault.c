...
static void writeChunk(FileVaultInfo* info) {
	...
	FLIPENDIAN(myChunk);
	HMAC_Init_ex(&(info->ctx), NULL, 0, NULL, NULL);
	HMAC_Update(&(info->ctx), (unsigned char *) &myChunk, sizeof(uint32_t));
	HMAC_Final(&(info->ctx), msgDigest, &msgDigestLen);
...
static void cacheChunk(FileVaultInfo* info, uint32_t chunk) {
	...
	FLIPENDIAN(chunk);
	HMAC_Init_ex(&(info->ctx), NULL, 0, NULL, NULL);
	HMAC_Update(&(info->ctx), (unsigned char *) &chunk, sizeof(uint32_t));
	HMAC_Final(&(info->ctx), msgDigest, &msgDigestLen);
...
void fvClose(AbstractFile* file) {
	...
	/* HMAC_CTX_cleanup(&(info->hmacCTX)); HMAC_CTX_cleanup() existed in OpenSSL versions before 1.1.0. */
	HMAC_CTX_free(&(info->ctx));
...
AbstractFile* createAbstractFileFromFileVault(AbstractFile* file, const char* key) {
	...
	/* HMAC_CTX_init(&(info->hmacCTX)); HMAC_CTX_init() was replaced with HMAC_CTX_reset() in OpenSSL versions 1.1.0. */
	HMAC_CTX_reset(&(info->ctx));
	HMAC_Init_ex(&(info->ctx), hmacKey, sizeof(hmacKey), EVP_sha1(), NULL);
